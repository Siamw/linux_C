...
Test(int id)
{
		if (State[id] == HUNGRY && State[LEFT(id)] != EATING 
				...
				if (sem_post(&Philosopher[id])<0/* sem_post */)  {
						perror("sem_post");
						pthread_exit(NULL);
				}
		}
...
PickUp(int id)
{
		...
		if (sem_post(&Mutex)<0/* sem_post */)  {
				perror("sem_post");
				pthread_exit(NULL);
		}

		if (sem_wait(&Philosopher[id])<0/* sem_wait */)  {
				perror("sem_wait");
				pthread_exit(NULL);
		}
}
...
main()
{
		...
		for (i = 0 ; i < NUM_MEN ; i++)  {
				if (sem_init(&Philosopher[i],0,0)<0/* sem_init */)  {
						perror("sem_init");
						exit(1);
				}
				id[i] = i;
		}
		if (sem_init(&Mutex,0,1)<0/* sem_init */)  {
				perror("sem_init");
				exit(1);
		}
		...
		for (i = 0 ; i < NUM_MEN ; i++)  {
				if (sem_destroy(&Philosopher[i])<0/* sem_destroy */)  {
						perror("sem_destroy");
				}
		}

		if (sem_destroy(&Mutex)<0/* sem_destroy */)  {
				perror("sem_destroy");
				exit(1);
		}
